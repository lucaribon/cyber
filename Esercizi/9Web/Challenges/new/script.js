...
var vara = ["", "fromCharCode", "length", "substr"];
function x(varb, varc) {
	var vard = [];
	var vare = "";
	for (z = 1; z <= 255; z++) {
		vard[fromCharCode(z)] = z
	};
	for (j = z = 0; z < varb.length; z++) {
		vare += fromCharCode(vard[varb.substr(z, 1) ^ vard[varc.substr(j, 1)]]);
		j = (j < varc.length) ? j + 1 : 0
	};
	return vare
}